(ns grads.handler
  (:require [compojure.core :refer [defroutes routes wrap-routes]]
            [grads.routes.home :refer [home-routes]]
            [grads.middleware :as middleware]
            [grads.session :as session]
...
(defn init
  ...
  (timbre/set-config!
    ...
    {:path "grads.log" :max-size (* 512 1024) :backlog 10})

  (if (env :dev) (parser/cache-off!))
  (start-nrepl)
  ;;start the expired session cleanup job
  (session/start-cleanup-job!)
  (timbre/info "\n-=[ grads started successfully"
...
(defn destroy
  ...
  []
  (timbre/info "grads is shutting down...")
  (stop-nrepl)
